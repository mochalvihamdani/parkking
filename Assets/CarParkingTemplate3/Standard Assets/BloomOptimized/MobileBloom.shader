...
Shader "Hidden/FastBloom" {
	...
	CGINCLUDE
		...
		{	
			...
			return color + tex2D(_Bloom, i.uv) * _Color.rgba;
		...
		{				
			...
			return max(color/4 - THRESHHOLD, 0) * ONE_MINUS_THRESHHOLD_TIMES_INTENSITY * _Color.rgba;
		}
		...
		half4 fragBlur8 ( v2f_withBlurCoords8 i ) : SV_Target
		{
			...
			half4 color = 0;
  			for( int l = 0; l < 7; l++ )  
  			{   
				half4 tap = tex2D(_MainTex, coords) * _Color.rgba;
				color += tap * curve4[l];
